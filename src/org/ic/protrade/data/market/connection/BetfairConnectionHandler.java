...
import org.ic.protrade.data.utils.MatchUtils;
...
public class BetfairConnectionHandler {
    ...
    private static Logger log = Logger
            .getLogger(BetfairConnectionHandler.class);
    ...
    private static void printTours(Tournament tournament) {
        ...
        for (LiveMatch m : tournament.getMatches()) {
            log.info("\t" + m.toString() + " --or-- "
                    + m.getEventBetfair().getName());
        }
    }
    ...
    public static List<Tournament> getTournamentsData() {
        ...
        for (EventMarketBetfair emb : events) {
            if (emb instanceof EventBetfair) {
                Tournament newTournament = new Tournament(emb.getName(),
                        (EventBetfair) emb);
    ...
    private static List<EventMarketBetfair> getEventData(int eventId) {
        ...
        try {
            ...
            if (events == null) {
                ...
                events = nonCouponEvents.toArray(new BFEvent[] {});
            }
            for (BFEvent e : events) {
                EventBetfair newTournament = new EventBetfair(e.getEventName(),
                        e.getEventId());
                newTournament.getChildren()
                        .addAll(getEventData(e.getEventId()));
                tournaments.add(newTournament);
            }
    ...
    private static void filterMatches(Tournament tournament) {
        List<LiveMatch> matches = getMatches(new ArrayList<LiveMatch>(), tournament.getEventBetfair());
        tournament.setMatches(matches);
    }
    
    private static List<LiveMatch> getMatches(List<LiveMatch> matches,
            EventBetfair eventBetfair) {
    ...
    public static ProfileData getProfileData() throws Exception {
        ...
        GetAccountFundsResp ukFunds = ExchangeAPI.getAccountFunds(Exchange.UK,
                apiContext);
        GetAccountFundsResp ausFunds = ExchangeAPI.getAccountFunds(
                Exchange.AUS, apiContext);
    ...
    private static void setProfileInfo(ProfileData profileData,
        ...
        profileData.setGamcareLossLimitFrequency(resp
                .getGamcareLossLimitFrequency().toString());
